...
private:
	string nume;
public:
		virtual void descrieTipCladire() = 0;
	
		tipCladire() {
			this->nume = "Cladirea este un/o: ";
		}
...
class Incapere {
		string nume;
	public:
		virtual void descrieTipIncapere() = 0;
	
		Incapere() {
			this->nume = "Incaperea este: ";
		}
...
private:
	...
	string* tipCamera;
public:
	void descrieTipCladire() {
		cout << "Cladirea are " << nrCamere << " camere si o suprafata de " << suprafata << " mp.";
	}
	//getteri
	...
	Casa(int nrCamere, float suprafata, string* tipCamera) :tipCladire(),idCasa(nrCasa) {
		nrCasa++;
		this->nrCamere = nrCamere;
		this->suprafata = suprafata;
		this->tipCamera = new string[nrCamere];
	...
	Casa(int nrCamere, string* tipCamera) :tipCladire(), idCasa(nrCasa) {
		nrCasa++;
		this->nrCamere = nrCamere;
	...
	Casa(const Casa& c) :tipCladire(c),idCasa(nrCasa++) {
...
private:
	...
	static int nrCamara;
public:
	void descrieTipIncapere() {
		cout << "Incaparea are "<<nrBorcane<<" borcane si are o latime de"<<latime<<" m.";
	}
	//getteri
	...
	Camara(int nrBorcane, string* tipBorcan) :Incapere(),idCamara(nrCamara) {
		nrCamara++;
		this->nrBorcane = nrBorcane;
		this->latime = 4;
		this->tipBorcan = new string[nrBorcane];
	...
	Camara(int nrBorcane, float latime, string* tipBorcan) :Incapere(),idCamara(nrCamara) {
		nrCamara++;
		this->nrBorcane = nrBorcane;
		this->latime = latime;
		this->tipBorcan = new string[nrBorcane];
	...
	Camara() :Incapere(),idCamara(nrCamara++) {
		this->nrBorcane = 0;
		this->latime = 5;
		this->tipBorcan = NULL;
	}
	Camara(const Camara& r) :Incapere(r),idCamara(nrCamara++) {
...
public:
	Obiecte() {
		this->nrObiecte = 10;
		this->cl = new tipCladire * [10];
		for (int i = 0; i < 10; i++) {
			this->cl[i] = new Casa;
		}
	}

	tipCladire*& operator[](int i) {
